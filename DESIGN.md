...
- **Main Black** (`#111111`): The foundation — primary text, button backgrounds, nav text, hero overlays. Deliberately not pure black (#000000), creating a fractionally softer reading experience
- **Main White** (`#FFFFFF`): Primary page canvas, button text on dark, card surfaces, nav bar background
...
- **Main Red** (`#D30005`): Critical errors, sale badges, urgent notifications (--podium-cds-color-red-600)
- **Bright Red** (`#EE0005`): Red-500, slightly lighter red for emphasis
- **Main Orange Badge** (`#D33918`): Badge text, promotional callouts (--podium-cds-color-text-badge)
...
Avoid UI gradients. When gradients appear, they are photographic — applied to product hero backgrounds (e.g., a red shoe on a red-to-deeper-red gradient). The design system itself is flat-color only.
...
**Display:** 
- Font: Helvetica Now Text Medium, Helvetica, Arial
...
|------|------|--------|-------------|----------------|-------|
| Display | 96px | 500 | 0.90 | — | Helvetica Now Display Medium, uppercase, hero headlines |
...
**Primary**
- Background: Main Black (`#111111`)
- Text: Main White (`#FFFFFF`), 16px/500, Helvetica Now Text Medium
- Border: none
...
- Background: Main White (`#FFFFFF`) — no visible card boundary in most cases
...
- Background: Main White (`#FFFFFF`), sticky
- Height: ~60px desktop
- Left: Company logo (24x24px SVG)
- Center: two lines, company name in first line and Category links in second line (16px/500 Helvetica Now Text Medium)
...
- Use Main Black (#111111) for all primary text — never pure #000000
...
- Don't add shadows to cards — The elevation model is entirely flat
...
- Don't soften the contrast — The design deliberately pushes black-on-white to maximum
...
- Primary CTA: Main Black (`#111111`)
- Background: Main White (`#FFFFFF`)
- Secondary surface: Light Gray (`#F5F5F5`)
- Heading text: Main Black (`#111111`)
- Body text / hover: Secondary Text (`#707072`)
- Border: Border Secondary (`#CACACB`)
- Error: Main Red (`#D30005`)
- Link: Link Blue (`#1151FF`)

### Example Component Prompts

- "Create a product hero section with full-bleed edge-to-edge photography, no border radius, a dark gradient overlay for text, and a massive uppercase 96px/500 headline in 0.90 line-height and a Main Black (#111111) pill button (30px radius)"
- "Design a 3-column product card grid with square images (no border radius), 4px gap between cards, product name in 16px/500 Main Black (#111111), price in 14px/500, and secondary text in Grey-500 (#707072)"
...
5. If something feels too colorful in the UI, it probably is — The design keeps UI greyscale
